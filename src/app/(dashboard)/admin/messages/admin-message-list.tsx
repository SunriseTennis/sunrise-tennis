...
import { replyToMessage, markMessageRead, archiveMessage } from './actions'
import { Card, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { MessageSquare, ChevronDown, ChevronUp, Reply, Clock, CheckCircle, Archive, User } from 'lucide-react'
...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  question_program: 'bg-purple-100 text-purple-700',
...
export function AdminMessageList({ messages }: { messages: AdminMessage[] }) {
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                  {msg.families && (
                    <span className="flex items-center gap-0.5">
                      ...
                      {msg.families.display_id} {msg.families.family_name}
                    </span>
                  )}
                  <Badge variant="secondary" className={cn('text-[10px] px-1.5 py-0', CATEGORY_COLORS[msg.category])}>
                    {CATEGORY_LABELS[msg.category] || msg.category}
                  </Badge>
                  {msg.players && <span>Re: {msg.players.first_name}</span>}
                  {msg.programs && <span>- {msg.programs.name}</span>}
              ...
              <CardContent className="border-t border-border pt-3 pb-4 space-y-3">
                ...
                <div className="rounded-lg bg-muted/50 px-3 py-2">
                  <p className="text-xs font-medium text-muted-foreground mb-1">
                    {msg.families ? `${msg.families.display_id} ${msg.families.family_name}` : 'Parent'} wrote:
                  </p>
                  ...
                  <>
                    {replyingTo === msg.id ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        <form action={async () => { await archiveMessage(msg.id) }}>
                ...
                {/* Archive already-replied messages */}
                {hasReply && !msg.archived_at && (
                  <form action={async () => { await archiveMessage(msg.id) }}>
                    <Button type="submit" size="sm" variant="ghost" className="gap-1.5 text-muted-foreground">
                      <Archive className="size-3.5" />
                      Archive
