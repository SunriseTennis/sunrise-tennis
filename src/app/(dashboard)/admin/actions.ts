...
export async function updateAttendance(
  ...
  if (session?.program_id) {
    ...
    for (const entry of entries) {
      ...
      //
      // When the same walk-in is later flipped to absent / noshow, void the
      // existing walk-in charge — the walk-in is being cancelled, the family
      // never contracted to attend via a booking, and there's no forfeit
      // policy for a non-rostered no-show. Without this void, admin sees
      // "marked absent" succeed but the family stays billed (Dion Gekas /
      // McAuley May 13 — see `.claude/rules/debugging.md` top-of-file).
      if (!booking) {
